...
for test1 in set1_test:
    ...
    for test2 in set2_test:
        set2.extend(test2)
        # 문제 조건을 만족한다면 해당 set을 저장한다
        if(IsItRight(set1, set2)):
            if not sorted([sorted(set1), sorted(set2)]) in solution:
                solution.append(sorted([sorted(set1), sorted(set2)]))
...
if len(solution)==0:
    ...
    print("%d개의 solution을 발견했습니다." %len(solution))
    for i in range(len(solution)):
        print("solution %d:" %i, solution[i])